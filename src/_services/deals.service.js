...
function getDeals({ skip, limit }) {
  ...
  if (typeof skip !== undefined && typeof limit !== undefined) {
    requestOptions.qs = { skip, limit };
  }
  if (requestOptions.qs) {
    url +=
      (url.indexOf("?") === -1 ? "?" : "&") + queryParams(requestOptions.qs);
...
function getDealBookings({ skip, limit }) {
  let apiUrl = process.env.REACT_APP_API_URL || "https://bot.prod.alibot.xyz";
  if (String(apiUrl).includes("localhost")) apiUrl = "http://localhost:8000";
  let url = apiUrl + `/backend/api/deals-bookings`;
  const requestOptions = {
    method: "GET",
    headers: authHeader(),
  };
...
function updateDeal(u) {
  ...
  const apiUrl =
    process.env.REACT_APP_API_URL || "https://aliserverbot.herokuapp.com";
  const url = apiUrl + `/backend/api/deals/`;
...
function deleteDealBooking(u) {
  const apiUrl =
    process.env.REACT_APP_API_URL || "https://aliserverbot.herokuapp.com";
  const url = apiUrl + `/backend/api/deals-bookings/`;
  const dealId = u.row._id || u.row.id;
  let update = {
    removed: true,
  };
    ...
    if (!response.ok) {
      ...
      if (errors) {
        errors.forEach((error) => {
          Toast.fire({
            type: "error",
            title: `${error.param} is ${error.msg}`,
          });
        });
      }
